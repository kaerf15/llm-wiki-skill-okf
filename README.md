...
**知识库是用户单独打开的一个 workspace 文件夹**（如 `~/Documents/OKF/`），**不是**本工具仓库，也**不需要**再套一层 `wiki/` 或 `wiki-okf/` 子目录。

```text
OKF/                       ← ★ workspace = 知识库根（BUNDLE_ROOT）
├── index.md
...
旧版把内容放在 `wiki/` 子目录；OKF 对齐后概念页直接在 bundle 根下。

## OKF 与创建规则

**Agent 仅在用户要求创建时才 scaffold。** 创建前先确认知识库类型（默认 `research`）。
...
```bash
# workspace 就是 bundle 根
python3 llm-wiki/scripts/scaffold.py ~/Documents/OKF "My AI Wiki" --type research
```
...
2. **新建一个空文件夹**（如 `~/Documents/OKF`），用 Cursor **打开它**作为 workspace。
3. 复制下面「部署提示词」整段发给 Agent。
4. 完成后在左侧应**直接**看到 `index.md`、`concepts/`、`raw/` — 无需再点进子文件夹。
...
知识库 (OKF bundle):
任意目录，例如 ~/Documents/OKF/  ← 打开此文件夹即为 bundle 根
```
...
- BUNDLE_ROOT = WORKSPACE（用户打开的空文件夹就是知识库根）
- 禁止在 WORKSPACE 下再建 wiki/、wiki-okf/ 等子文件夹当知识库
- 若 workspace 含 llm-wiki/、web/、audit-shared/（工具项目）→ 停止并提示用户另建空文件夹
...
3. 若 BUNDLE_ROOT 尚无 OKF 结构：
   python3 <TEMP>/llm-wiki/scripts/scaffold.py "<BUNDLE_ROOT>" "<主题>" --type <KB_TYPE>
4. 复制 <TEMP>/llm-wiki 到 <BUNDLE_ROOT>/.agents/skills/llm-wiki
5. Web 已部署则只注册 BUNDLE_ROOT；未部署则安装 Web 到 APP_ROOT 并构建
6. 注册 BUNDLE_ROOT 到 wikis.json；可选安装 MarkItDown
7. 验证 workspace 根有 index.md（含 okf_version）、skill 存在、http://127.0.0.1:4875 可访问
8. 删除临时 clone

【完成后汇报】
- 知识库目录 = workspace 根 <BUNDLE_ROOT>
- 左侧应直接看到 index.md、concepts/、raw/（不是子文件夹里的）
...
```bash
mkdir -p ~/OKF && cd ~/OKF
python3 /path/to/llm-wiki-skill-okf/llm-wiki/scripts/scaffold.py . "My Research Topic" --type research
cp -r /path/to/llm-wiki-skill-okf/llm-wiki .agents/skills/llm-wiki
```
...
python3 llm-wiki/scripts/import_source.py my-paper.pdf ~/OKF --kind papers
```
...
```bash
python3 llm-wiki/scripts/lint_wiki.py ~/OKF
python3 llm-wiki/scripts/audit_review.py ~/OKF --open
```
...
Web viewer 负责阅读体验：只显示链接文字、SPA 内导航、反向链接、知识图谱、死链高亮。亦支持 `wiki/...` 形式的历史链接路径（legacy 迁移用）。
...
```json
{
  "defaultWikiId": "okf",
  "wikis": [
    { "id": "okf", "name": "AI Research", "path": "/Users/you/Documents/OKF" },
    { "id": "sales-catalog", "name": "Sales Catalog", "path": "/Users/you/wikis/sales-catalog" }
  ]
}
```

URL 深链：`http://127.0.0.1:4875/?wiki=okf&page=index.md`
...
**知识库**（用户 workspace，不在本仓库内）：

```text
<workspace>/           # 例如 ~/Documents/OKF
├── index.md           # okf_version: "0.1"
├── log.md
├── AGENTS.md · CLAUDE.md · BUNDLE.md
├── .agents/skills/llm-wiki/
├── concepts/ · entities/ · summaries/
